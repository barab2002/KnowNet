...
export const SemanticSearchPage = () => {
  ...
  const renderHighlightedSnippet = (snippet: string) => {
    ...
    // Create a regex to find any of the query words
    ...
    return parts.map((part, i) =>
      queryWords.some((w) => w.toLowerCase() === part.toLowerCase()) ? (
        <mark
          key={i}
          className="bg-emerald-500/30 text-emerald-900 dark:text-emerald-100 rounded px-0.5 font-bold"
        >
          {part}
        </mark>
      ) : (
            ...
            <div className="mt-8 space-y-6">
              ...
              {results.map((post) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    {post.matchedTags.length > 0 && (
                      <div className="flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400 font-semibold bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded-full">
                        ...
                        {post.matchedTags.length} tag
                        {post.matchedTags.length > 1 ? 's' : ''} matched
                      </div>
                    )}
                  </div>
                  ...
                  {post.matchSnippet && (
                    <div className="bg-emerald-500/10 rounded-lg p-3 border-l-4 border-emerald-500 mb-4 animate-in fade-in slide-in-from-left-2 duration-300">
                      <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 text-xs font-bold mb-1 uppercase tracking-wider">
                        <span className="material-symbols-outlined text-xs">
                          target
                        </span>
                        Direct Search Match
                      ...
                      <p className="text-sm text-slate-700 dark:text-slate-300 font-mono">
                        {renderHighlightedSnippet(post.matchSnippet)}
                      </p>
                    </div>
                  )}
